...
public class RCCCarControllerV2 : MonoBehaviour {
	...
	void Start (){
        ...
        idleEngineTorqueCurve.AddKey(Mathf.Lerp(0.0f, 6.0f, 0.5f), 0f);
        idleEngineTorqueCurve.AddKey(Mathf.Lerp(0f, 8f, 0.5f), .75f);
		...
		if(changingGear){
			...
			if(!reversing){
                if (leftSide)
                {
                    ...
                    //Debug.Log("first = " + Mathf.Clamp(motorInput * differentialRatioLeft, 0f, 1f) + " engineTorqueCurve = " + engineTorqueCurve[currentGear].Evaluate(speed));
                }
                else
